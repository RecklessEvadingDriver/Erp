...
### Quick deploy (single Render flow)

1. Push this repository to GitHub.
2. In Render, click **New + → Blueprint** and select this repo.
3. Render reads `render.yaml` and auto-configures build/start/env for you.
4. Only add **one required secret** during setup: `MONGO_URI` (your Atlas URI).
   - `JWT_SECRET` is auto-generated by Render (`generateValue: true`).
   - `CLIENT_URL` is auto-derived from the deployed Render service host.
   - Cloudinary vars are optional (only needed for upload endpoints).
